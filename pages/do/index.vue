...
export default {
  ...
  async fetch() {
    ...
    const response = await this.$axios.get(
      `death_certificate_form/${this.genPage()}${this.genSearch()}${this.genStatus()}${this.genFilterDate()}`
    );
  ...
  methods: {
    ...
    async getCount(status) {
      const result = await this.$axios.get(`death_certificate_form/count/${status}`);
      return result.data;
    },
    async getForms() {
      this.isBusy = !this.isBusy;
